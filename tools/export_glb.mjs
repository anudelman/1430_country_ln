...
/** Re-check an already-written .glb without paying for the export again. */
const VERIFY_ONLY = has('verifyonly');
...
try {
  ...
  if (VERIFY_ONLY) {
    // Still needs a page on the right origin so the module imports resolve.
    await page.goto(`${srv.url}/app/index.html?shot=1&quality=thumb`, { waitUntil: 'domcontentloaded' });
    console.log(`verify-only: re-checking ${path.relative(ROOT, OUT)}`);
  } else {
  ...
  if (VERIFY) {
    // Load the file back over HTTP rather than passing the buffer into the
    // page. The first version base64'd 197 MB out and then handed the same
    // 263 MB string back in, which OOM'd the renderer and killed the browser
    // mid-verify. The server already serves the repo root, so a URL costs
    // nothing.
    ...
    if (expectedMeshes && ok.meshes !== expectedMeshes) {
      console.log(`  WARNING: mesh count changed on round trip ` +
        `(${expectedMeshes} -> ${ok.meshes}) — something did not serialise`);
    }
  }
